...
const RatingCodec=(()=>{
  ...
  function fromGame(g,seat=0){
    ...
    if(g.mode==='solo'&&g.scoreVersion===1&&!g.players[0].cpu&&g.players[1].cpu&&g.players[0].alive!==g.players[1].alive&&opponents.includes(g.players[1].persona)){
      return {id:g.matchId,layout:g.layout,opponent:g.players[1].persona,won:g.players[0].alive,date:Math.floor(g.finishedAt/1000)};
    }
    if(g.mode!=='solo'&&Number.isInteger(seat)&&seat>=0&&seat<2&&!g.players.some(p=>p.cpu)&&g.players[0].alive!==g.players[1].alive){
      return {id:g.matchId,layout:g.layout,opponent:'human',won:g.players[seat].alive,date:Math.floor(g.finishedAt/1000)};
    }
    return null;
  }
  ...
  function standings(events){
    ...
    for(const e of merge([],events).events){
      ...
      const before=ratings[ladder],bot=ratings[opponent];
      const expected=1/(1+10**((bot-before)/400));
      const raw=K*((e.won?1:0)-expected);
      const delta=raw>0?Math.min(raw,bot):Math.max(raw,-before);
      ratings[ladder]+=delta;ratings[opponent]-=delta;
      records[ladder][e.won?'wins':'losses']++;
      if(e.opponent!=='human')records[e.opponent][e.won?'losses':'wins']++;
      changes[e.id]={before,after:ratings[ladder],delta,ladder};
    }
