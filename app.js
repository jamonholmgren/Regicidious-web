...
function renderTextWaiting() {
  ...
  const invites=textAccess()===0?`<section class="panel"><h2>Summon each kingdom</h2><p class="small muted">Send each ruler their sealed summons once. Opening it claims that seat on their device. Then keep every turn in one group chat, where the whole war council can witness it.</p>${game.players.map((q,i)=>i===0?'':`<p>${escapeHTML(q.name)} ${SUITS[i]}</p>${inviteLinks[i]?.url?`<div class="actions"><button class="button secondary" data-action="copy-invite" data-index="${i}">Copy summons</button><button class="button secondary" data-action="share-invite" data-index="${i}">Send summons…</button></div><textarea readonly rows="2">${escapeHTML(inviteLinks[i].url)}</textarea>`:'<p class="small muted">Sealing summons…</p>'}`).join('')}</section>`:'';
  const settingUp=game.phase==='setup';
  const summary=`<section class="waiting-summary"><div class="crown">${p.emoji}</div><div><div class="phase">${settingUp?`Battle lines · ${game.setup+1} of ${game.players.length}`:`Text multiplayer · turn #${game.turnNumber}`}</div><h1>${escapeHTML(p.name)}’s ${settingUp?'battle lines':'turn'}</h1><p>Share the next dispatch in thy group chat. It is addressed to ${escapeHTML(p.name)}, and every ruler may carry it there.</p>${playerChips(game)}<p class="muted small">Started ${escapeHTML(game.startedAt>0?dateText(game.startedAt):'unknown')}</p>${dispatchNotice?.matchId===game.matchId?`<p class="status small">${escapeHTML(dispatchNotice.text)}</p>`:''}</div></section>`;
  const actions=turnLink?`<div class="actions"><button class="button" data-action="share-turn">Share turn to group chat</button><button class="button secondary" data-action="copy-turn">Copy dispatch</button></div><details class="share-detail"><summary>Show dispatch and link</summary><textarea readonly rows="5" aria-label="Dispatch and link">${escapeHTML(shareMessage(game,turnLink))}</textarea></details>`:'<p class="muted small">Preparing a private turn link…</p>';
...
function renderTextInvites() {
  ...
  frame(`<section class="panel dispatch"><div class="phase">Gather your kingdoms</div><h1>Send the royal invitations</h1><p>Each player needs their own named invite to claim a seat. Send these before declaring war, then make one group chat for the campaign—every future dispatch belongs there.</p>${seats}<p class="flavor">The gauntlet has been thrown down.</p><button class="button wide" data-action="declare-war">Declare war!</button></section>`);
}
